...
function mockChatResponse(body: { marks_awarded: number; reasoning: string; confidence: string }) {
...
describe('markExtendedPart', () => {
  ...
  it('returns the parsed result with needs_review false for high confidence', async () => {
    mockCreate.mockResolvedValue(mockChatResponse({ marks_awarded: 2, reasoning: 'Met M1 and A1.', confidence: 'high' }));
  ...
  it('sets needs_review true for low confidence', async () => {
    mockCreate.mockResolvedValue(mockChatResponse({ marks_awarded: 0, reasoning: 'Ambiguous answer.', confidence: 'low' }));
    const result = await markExtendedPart(baseInput, new AbortController().signal);
    expect(result.needs_review).toBe(true);
  });

  it('leaves needs_review false for medium confidence', async () => {
    mockCreate.mockResolvedValue(mockChatResponse({ marks_awarded: 1, reasoning: 'Partial credit.', confidence: 'medium' }));
    const result = await markExtendedPart(baseInput, new AbortController().signal);
    expect(result.needs_review).toBe(false);
  });

  it('clamps marks_awarded above the maximum', async () => {
    mockCreate.mockResolvedValue(mockChatResponse({ marks_awarded: 99, reasoning: 'x', confidence: 'medium' }));
    const result = await markExtendedPart(baseInput, new AbortController().signal);
    expect(result.marks_awarded).toBe(3);
  });

  it('clamps a negative marks_awarded to zero', async () => {
    mockCreate.mockResolvedValue(mockChatResponse({ marks_awarded: -5, reasoning: 'x', confidence: 'medium' }));
    const result = await markExtendedPart(baseInput, new AbortController().signal);
  ...
  it('throws when the model refuses', async () => {
    mockCreate.mockResolvedValue({ choices: [{ message: { refusal: 'Cannot assist with this.' } }] });
    await expect(markExtendedPart(baseInput, new AbortController().signal)).rejects.toThrow('declined');
  });
});
